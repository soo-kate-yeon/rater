...
async def test_list_sets(test_client: AsyncClient, test_set_fixture: Set):
    """Set 목록 조회 테스트"""
    response = await test_client.get("/api/v1/sets")
...
async def test_list_sets_with_filter(test_client: AsyncClient, test_set_fixture: Set):
    """Set 목록 필터 조회"""
    response = await test_client.get(f"/api/v1/sets?source={test_set_fixture.source}")
    assert response.status_code == 200
    data = response.json()
...
async def test_list_sets_pagination(test_client: AsyncClient):
    """Set 목록 페이징"""
    response = await test_client.get("/api/v1/sets?skip=0&limit=5")
    assert response.status_code == 200
    data = response.json()
    assert data["skip"] == 0
    assert data["limit"] == 5
...
async def test_get_set(test_client: AsyncClient, test_set_fixture: Set):
    """단일 Set 조회"""
    response = await test_client.get(f"/api/v1/sets/{test_set_fixture.id}")
    assert response.status_code == 200
    data = response.json()
...
async def test_get_set_not_found(test_client: AsyncClient):
    ...
    response = await test_client.get(f"/api/v1/sets/{fake_id}")
...
async def test_get_set_invalid_uuid(test_client: AsyncClient):
    """잘못된 UUID 형식 시 400"""
    response = await test_client.get("/api/v1/sets/invalid-uuid")
...
async def test_update_set(test_client: AsyncClient, test_set_fixture: Set):
    ...
    response = await test_client.patch(
        f"/api/v1/sets/{test_set_fixture.id}",
...
async def test_update_set_not_found(test_client: AsyncClient):
    ...
    fake_id = str(uuid.uuid4())
    response = await test_client.patch(f"/api/v1/sets/{fake_id}", json={"title": "New"})
    assert response.status_code == 404
...
async def test_delete_set(test_client: AsyncClient, test_set_fixture: Set):
    """Set 삭제"""
    response = await test_client.delete(f"/api/v1/sets/{test_set_fixture.id}")
    assert response.status_code == 204

    # 삭제 확인
    get_response = await test_client.get(f"/api/v1/sets/{test_set_fixture.id}")
...
async def test_delete_set_not_found(test_client: AsyncClient):
    ...
    fake_id = str(uuid.uuid4())
    response = await test_client.delete(f"/api/v1/sets/{fake_id}")
    assert response.status_code == 404
