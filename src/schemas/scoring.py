...
class DeliverySignals(BaseModel):
    ...
    secpchk: float = Field(default=0.0, description="평균 chunk 길이 (초, segment 기반)")
    silpsecutt: float = Field(default=0.0, description="초당 pause 빈도 (pauses per second)")
    within_clause_interruptions: int = Field(default=0, description="절 내부 중단 횟수 (IPC)")
