...
down_revision: str | Sequence[str] | None = "20260127_schema"
...
def upgrade() -> None:
    ...
    op.create_index(
        op.f("ix_independent_topics_number"), "independent_topics", ["number"], unique=True
    )
    op.create_index(
        op.f("ix_independent_topics_source"), "independent_topics", ["source"], unique=False
    )
